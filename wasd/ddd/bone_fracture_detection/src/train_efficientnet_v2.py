...
class EfficientNetV2Trainer:
    """
    EfficientNetV2-S trainer with GPU optimization
    """
    ...
    def build_model(self, freeze_base=False):
        """
        Build EfficientNetV2-S model with custom classification head
        ...
        # Create model
        self.model = models.Model(inputs=inputs, outputs=outputs, name='EfficientNetV2S')
    ...
    def create_callbacks(self, model_dir, logs_dir):
        ...
        checkpoint_path = model_dir / 'efficientnetv2s_best.h5'
        ...
        tensorboard = TensorBoard(
            log_dir=str(logs_dir / f'efficientnetv2s_{datetime.now().strftime("%Y%m%d_%H%M%S")}'),
            histogram_freq=1,
            write_graph=True,
            update_freq='epoch'
        )
    ...
    def train(self, train_ds, val_ds, epochs=50, callbacks=None):
        ...
        print("\n" + "="*80)
        print("✅ TRAINING COMPLETE - EFFICIENTNETV2-S")
        print("="*80)
    ...
    def evaluate(self, test_ds):
        ...
        print("\n" + "="*80)
        print("📊 EVALUATING MODEL - EFFICIENTNETV2-S")
        print("="*80)
...
def main():
    ...
    print("Testing EfficientNetV2-S trainer...")
    
    trainer = EfficientNetV2Trainer(img_size=224, num_classes=2, learning_rate=1e-4)
    model = trainer.build_model(freeze_base=False)
    trainer.compile_model()
    
    print("\n✅ EfficientNetV2-S trainer initialized successfully!")
